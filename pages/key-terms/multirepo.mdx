...
In these large companies, there can end up being so much code that it becomes really difficult to keep it all in one repository. Things start to perform poorly because *there's just too much darn code*. Builds, pipelines, and <KeyTermLink termUrl="version-control">version control</KeyTermLink> can all start to suffer from the sheer mass of code.
